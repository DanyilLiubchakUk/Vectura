...
export async function findNearestTradingDaysInRange(
  ...
  if (!dbResults[centerDay]) priorityDays.push(centerDay);
  ...
  for (
    ...
    const day = daysToCheck[i];
    if (!dbResults[day] && !priorityDays.includes(day)) priorityDays.push(day);
  }
  ...
  for (const day of missingDays) {
    ...
    if (!priorityDays.includes(day)) priorityDays.push(day);
  }
  ...
  if (!previous && centerIndex > 0) {
    ...
    if (additionalDaysToCheck.length > 0) {
      ...
      additionalChecks.forEach(({ day, exists }) => {
        allResults[day] = exists;
        if (exists && !previous) previous = day;
      });
    }
  }
